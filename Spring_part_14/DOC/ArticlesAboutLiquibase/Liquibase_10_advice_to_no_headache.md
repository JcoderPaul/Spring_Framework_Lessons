...
с Liquibase» см. ["Управление миграциями БД с Liquibase"](./DBMigrationManagement.md).
...
**Подробнее:** 
- **1.1** Чейнджлоги изменений базы данных должны содержаться в отдельных папках и соответствуют версиям приложения.
    - Плюсы: Всегда можно легко сказать, какие изменения схемы данных произошли при переходе с версии на версию приложения. Поддерживается аккуратность в папках и не позволяет расслабляться.
