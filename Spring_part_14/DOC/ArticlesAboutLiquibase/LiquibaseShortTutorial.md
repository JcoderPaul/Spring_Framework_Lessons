...
Liquibase — один из самых универсальных инструментов для миграции баз данных см. ["Миграция базы данных: что это такое и как это делается"](../DatabaseMigration/DatabaseMigration.md). 
Он работает практически на всех платформах баз данных SQL. Так что, используете ли вы MySQL, SQL Server, Oracle, Firebird или
...
В документации Liquibase есть примечания о поддержке каждой из применяемых баз данных, см. [« addColumn »](https://docs.liquibase.com/change-types/add-column.html), 
например, автоматически откатится, поэтому нет необходимости включать откат, показанный в примере выше. Со схемой можно делать практически все, что угодно, включая, 
среди прочего, манипулирование представлениями, хранимыми процедурами, функциями и триггерами.
...
Надеюсь, вам понравилось это введение в управление миграцией баз данных с помощью Liquibase. 
Лучше [прочитать документацию см.](https://docs.liquibase.com/home.html), чтобы получить полную 
информацию о том, как выполнять обновления.
