...
```
       ...
       в котором отсутствуют какие-либо ограничения, налагаемые предыдущей работой. Пример аналогичен
...
добавили новые поля в структуре БД и при это несете ответственность за производство веб-приложения для электронной
коммерции.

Предположим, что в этом приложении у вас есть клиенты, и у них есть данные о своих клиентах. Еще в бурные дни «зеленых
полей» вы приняли прискорбное решение хранить в своей базе данных перечисление под названием «CustomerType», и в
результате это привело к вводу ключей в некоторые другие вспомогательные таблицы, которые генерируют много хлама
в вашей схеме.
...
достаточно уверенно. Пользователи начинают пользоваться обновлением, приходят новые клиенты, они в системе, и новые данные 
начинают поступать.
...
одну вещь**, которая может показаться неочевидной: [**ФЛАГ ФУНКЦИИ**](https://www.cloudbees.com/topics/feature-flags)
```
       Что такое флаг функции?
       Флаги функций — это программные средства, позволяющие отключать функции без изменения исходного кода или повторного 
       развертывания. Их часто называют переключателями функций, переключателями релизов или переключателями функций. "Флаг 
       функции" представляет собой простое условное выражение (например, «если» — «если»). Этот флаг функции определяет, 
       когда происходит выполнение кода. Это позволяет развернуть новые функции, которые не будут отображаться для пользователей.
```
См. более подробно: ["Feature Toggle (Or Flipper): What Is It and Why Use It?"](https://www.cloudbees.com/blog/what-is-feature-toggle)
...
При миграции я рекомендую вам принять гибкую концепцию см. ["How Thin is Thin?" An Example of Effective Story Slicing"](https://dzone.com/articles/how-thin-thin-example) маленьких
