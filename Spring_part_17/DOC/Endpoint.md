### Endpoint: определение и принцип работы (по разному об одном и том же).
...
Endpoint имеет уникальный URL (Uniform Resource Locator), который указывает на конкретное место в некотором ресурсе, например:
...
использоваться для фильтрации данных или задания определенных условий. Например, endpoint - `/products?category=electronics`, 
может возвращать только электронику из каталога товаров и все.
