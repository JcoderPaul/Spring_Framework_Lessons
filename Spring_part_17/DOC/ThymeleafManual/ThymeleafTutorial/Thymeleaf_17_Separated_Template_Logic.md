...
файлом, находящимся в том же месте (например, папке) в качестве файла шаблона с тем же именем, но с расширением *.th.xml:
...
Таким образом, файл [home.html](https://github.com/thymeleaf/thymeleafexamples-gtvg/blob/3.1-master-jakarta/src/main/webapp/WEB-INF/templates/home.html) может быть полностью логичным и может выглядеть так:
...
За исключением [StringTemplateResolver](https://javadoc.io/doc/org.thymeleaf/thymeleaf/latest/org/thymeleaf/templateresolver/StringTemplateResolver.html) (который не допускает разделенной логики), все другие готовые версии [ITemplateResolver](https://javadoc.io/doc/org.thymeleaf/thymeleaf/latest/org/thymeleaf/templateresolver/ITemplateResolver.html) будут предоставлять флаг под названием useDecoupledLogic, который будет отмечать все шаблоны, найденные этим резольвером, как потенциально имеющие всю или часть своей логики в отдельном ресурсе:
...
определяется точкой расширения, [IDecoupledTemplateLogicResolver](https://javadoc.io/doc/org.thymeleaf/thymeleaf/latest/org/thymeleaf/templateparser/markup/decoupled/IDecoupledTemplateLogicResolver.html), для
которой предоставляется реализация по умолчанию: [StandardDecoupledTemplateLogicResolver](https://javadoc.io/doc/org.thymeleaf/thymeleaf/latest/org/thymeleaf/templateparser/markup/decoupled/StandardDecoupledTemplateLogicResolver.html).
...
[См. далее →](../ThymeleafTutorial/Thymeleaf_18_Appendix_A.md)
