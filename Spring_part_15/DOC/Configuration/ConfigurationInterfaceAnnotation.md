- [Исходник статьи](https://docs.spring.io/spring-framework/docs/current/javadoc-api/org/springframework/context/annotation/Configuration.html)

---
### Annotation Interface Configuration

---
**Пакет:** [org.springframework.context.annotation](https://docs.spring.io/spring-framework/docs/current/javadoc-api/org/springframework/context/annotation/package-summary.html)
...
Дополнительные сведения см. в документации [AnnotationConfigApplicationContext](https://docs.spring.io/spring-framework/docs/current/javadoc-api/org/springframework/context/annotation/AnnotationConfigApplicationContext.html), а инструкции по веб-конфигурации в контейнере сервлетов 
см. в разделе [AnnotationConfigWebApplicationContext](https://docs.spring.io/spring-framework/docs/current/javadoc-api/org/springframework/web/context/support/AnnotationConfigWebApplicationContext.html).
...
компонентов (обычно с использованием элемента <context:comComponent-scan/> Spring XML) и, следовательно, также могут использовать 
преимущества @Autowired/@Inject, как и любой обычный `@Component`. В частности, если присутствует один конструктор, семантика 
автоматического связывания будет применяться прозрачно для этого конструктора:
...
Подробности смотрите в документации [@ComponentScan](https://docs.spring.io/spring-framework/docs/current/javadoc-api/org/springframework/context/annotation/ComponentScan.html)
...
получения подробной информации](https://docs.spring.io/spring-framework/docs/current/javadoc-api/org/springframework/context/annotation/Bean.html).
...
См. так же:
- Раздел ниже о создании классов `@Configuration` с помощью Spring XML с использованием `@ImportResource`;
...
Подробности смотрите в справочной документации [по платформе TestContext](https://docs.spring.io/spring-framework/reference/testing/testcontext-framework.html).
...
- `boolean enforceUniqueMethods` - Укажите, должны ли методы @Bean иметь уникальные имена методов, в противном случае бросается исключение, чтобы предотвратить случайную перегрузку.
...
- `boolean proxyBeanMethods` - Укажите, следует ли использовать методы @Bean для обеспечения поведения жизненного цикла компонента, например, для возврата общих экземпляров одноэлементного компонента даже в случае прямых вызовов метода @Bean в пользовательском коде.
...
объявлены в классах, отличных от @Configuration, то есть «@Bean Lite Mode» (см. док. [@Bean](https://docs.spring.io/spring-framework/docs/current/javadoc-api/org/springframework/context/annotation/Bean.html)). Таким образом, с точки зрения поведения это эквивалентно устранению стереотипа @Configuration.

- `String value` - Явно укажите имя определения bean-компонента Spring, связанного с @Configuration классом. Если не указать (обычный случай), имя компонента будет сгенерировано автоматически.
