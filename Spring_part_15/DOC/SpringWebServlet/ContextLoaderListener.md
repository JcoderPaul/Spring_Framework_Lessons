...
---
Прослушиватель начальной загрузки для запуска и завершения работы корневого WebApplicationContext Spring. Просто делегирует ContextLoader, а также ContextCleanupListener.

ContextLoaderListener поддерживает внедрение корневого контекста веб-приложения через конструктор ContextLoaderListener(WebApplicationContext), что позволяет программно настраивать инициализаторы
сервлетов.

Примеры использования см. в разделе WebApplicationInitializer.

---
Поля, унаследованные от класса org.springframework.web.context.ContextLoader: CONFIG_LOCATION_PARAM, CONTEXT_CLASS_PARAM, CONTEXT_ID_PARAM, CONTEXT_INITIALIZER_CLASSES_PARAM, GLOBAL_INITIALIZER_CLASSES_PARAM
