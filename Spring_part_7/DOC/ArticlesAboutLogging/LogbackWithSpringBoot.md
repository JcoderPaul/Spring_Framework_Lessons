...
простые, но мощные параметры конфигурации и требует небольшого объема памяти. 

Я представил logback в статье [LoggingBegin](./LoggingBegin.md)
...
способов экстернализации свойств конфигурации Spring Boot. 

В структуре по умолчанию веб-приложения Spring Boot вы можете найти файл application.properties в папке Resources. 
В файле application.properties вы можете определить уровни логирования Spring Boot, средства логирования приложений, 
Hibernate, Thymeleaf и т. д.
...
Также существует свойство logging.path, позволяющее указать путь к файлу журнала.
...
В выводе отладочные сообщения и сообщения более высокого уровня IndexController зашел в консоль и файл. 
Это потому, что в файле application.properties, мы указали ОТЛАЖИВАТЬ (DEBUG) в качестве уровня журналирования для пакета
...
Spring Boot файл конфигурации logback-spring.xml должен находиться в пути к классам.
...
Поддержка конфигурирования Logback при помощи <springProfile> доступна начиная с версии SpringBoot 1.3.0.M2.
