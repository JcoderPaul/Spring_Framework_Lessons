...
public class UserController {
    ...
    */
    ...
    /*
    Помним, что поиск, создание, обновление и удаление мы регулируем через тип HTTP запроса, это
    может быть, как POST, так или PUT вариант запроса.

    Lesson 90 - Аннотируем UserCreateEditDto как @Validated, тем самым запускаем процесс валидации
    и 'активируем' аннотации размещенные над полями в spring/oldboy/dto/UserCreateEditDto.java.
    Добавляем объект BindingResult для отлавливания всех ошибок валидации для последующего их
    отображения на стороне пользователя.

    Lesson 92 - Добавляем группы в параметры аннотации @Validate, Default.class - определяет все
    аннотации предоставленные Spring, т.е. она запускает все аннотации, которые не содержат группы.
    CreateAction.class - группа создания записей.

    При обращении с GET запросом к http://localhost:8080/users через браузер пользователь приложения
    обращается (или его запрос обрабатывает) к методу - *.findAll() и в аннотации @GetMapping
    аргументы явно не указаны они вынесены в аннотацию к самому классу контроллера @RequestMapping и
    являются общими с методом *.create(). С ОДНИМ ВАЖНЫМ отличием - тут метод запроса POST.

    Для профессионалов и слегка прошаренных все выше описанное очевидно, но для новичка, чем больше
    повторений - тем лучше для понимания и запоминания материала.
    */
    @PostMapping
