...
избавиться от [ResponseEntity<?>](https://docs.spring.io/spring-framework/docs/current/javadoc-api/org/springframework/http/ResponseEntity.html) 
в контроллерах не получится, и необходимо написать целую статью. Для начала, немного введения.
...
его Id в качестве параметра [@RequestParam](https://docs.spring.io/spring-framework/docs/current/javadoc-api/org/springframework/web/bind/annotation/RequestParam.html). 

Например, наш код выглядит таким образом:
...
![500_STATUS_ERROR.png](../ResponseEntity/500_STATUS_ERROR.png)
...
или, что скорее всего вероятно - фронту, JSON, который бы более-менее подробно описывал ошибку. 

Выглядит такой код примерно так:
...
чтобы фронтенд мог понимать, что именно пошло не так. 

Например, при корректном исполнении программы, передавая id=1, мы увидим просто успешно переданный объект Product 
с кодом 200, а вот в случае продукта с id = 299 результат уже будет такой см. 

![404_HTTP_ERROR.png](../ResponseEntity/404_HTTP_ERROR.png)

Всё ещё не красиво, но, уже хотя бы понятно, что продукт не найден. Мы имеем статус код 404 и фронт уже как-то
может с этим работать. Это здорово, но нам бы хотелось более конкретного описания ошибки и результата. 

Давайте, в таком случае, создадим новый класс:
...
**Отлично! Этого мы и хотели.**

Стало понятно, в чём проблема. Фронтенд легко распарсит этот JSON и обработает наше сообщение. Однако, сам метод контроллера 
теперь выглядит не слишком красиво. Да и когда сталкиваешься с чужим кодом, любой из нас сразу хотел бы видеть тип объекта, 
который будет возвращаться, а не какой-то там ResponseEntity со знаком вопроса в скобочках.
...
Начнём сверху вниз:
- [@ControllerAdvice](https://docs.spring.io/spring-framework/docs/current/javadoc-api/org/springframework/web/bind/annotation/ControllerAdvice.html) - используется для глобальной обработки ошибок в приложении Spring. То есть любой Exception, выпадающий в нашем приложении, будет замечен нашим ControllerAdvice.
- [@Slf4j](https://www.baeldung.com/slf4j-with-log4j2-logback) - используется для логгирования, заострять внимание мы на этом не будем.
- Далее создаём собственный класс, назвать его можем как угодно, пусть будет GlobalExceptionHandler.

**И вот тут уже интересное!**

- [@ExceptionHandler](https://docs.spring.io/spring-framework/docs/current/javadoc-api/org/springframework/web/bind/annotation/ExceptionHandler.html) - аннотирует метод *.catchResourceNotFoundException(). Эта аннотация позволяет нам указать, что мы хотим перехватывать и обрабатывать исключения определённого типа, если они возникают, и зашивать их в ResponseEntity, чтобы вернуть ответ нашему фронтэнд разработчику. В аргументах метода
указываем, какую именно ошибку мы собираемся ловить.

В данном случае, это наш кастомный ResourceNotFoundException. И возвращать мы будем точно такой же ResponseEntity, как и в примере выше, однако, прописываем мы его ВСЕГО 1 раз - в этом классе.

Spring на этапе обработки этой ошибки самостоятельно поймёт, что в методе нашего контроллера вместо нашего класса Product нужно будет вернуть ResponseEntity.
...
---
См. [комментарии к статье в русском исходнике](https://habr.com/ru/articles/675716/).
