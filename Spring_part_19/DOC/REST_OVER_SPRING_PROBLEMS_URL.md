...
наилучшего представления ресурса в [ContentNegotiatingViewResolver](https://docs.spring.io/spring-framework/docs/current/javadoc-api/org/springframework/web/servlet/view/ContentNegotiatingViewResolver.html).
...
Проблема усугубляется желанием использовать слеш или звездочку в качестве маркера коллекций,
то есть вместо `/service/hotels` в данном случае кто-то мог бы использовать, к примеру `/service/hotel/` 
или `/service/hotel/*`, как URL для списка всех отелей, хоть это и не очень интуитивно и еще менее расширяемо.
...
Теперь давайте вместе подумаем, что происходит при создании контекста. 

Как только Spring видит: `<mvc:annotation-driven />`, он создает один DefaultAnnotationHandlerMapping и помещает его 
в контекст. Как только он видит определенный выше bean, он создает другой экземпляр DefaultAnnotationHandlerMapping, и 
также помещает его в контекст. Так что наше крутое приложение будет иметь два экземпляра DefaultAnnotationHandlerMapping, 
один с настройками по-умолчанию, а другой настроен так, как нужно.

Какой [HandlerMapping](https://docs.spring.io/spring-framework/docs/current/javadoc-api/org/springframework/web/reactive/HandlerMapping.html) 
будет обрабатывать HTTP-запрос первым зависит от их внутреннего порядка, и вне вашего контроля
...
вы будете использовать [ContentNegotiatingViewResolver](https://docs.spring.io/spring-framework/docs/current/javadoc-api/org/springframework/web/servlet/view/ContentNegotiatingViewResolver.html), 
и вести переговоры с клиентом о лучшем для него представлении ресурсов. И в этом случае вы фактически потеряли контроль над этим 
занятным и важным в REST процессом для неявно созданных маппингов. Результатом запроса к ним может быть как правильное представление, 
так и неправильное, в некоторых ситуациях результатом будет исключение и 500.
...
`<mvc:annotation-driven />` и регистрировать AnnotationMethodHandlerAdaptor руками:
...
---
См. [комментарии к статье в исходнике](https://habr.com/ru/articles/111902/).
