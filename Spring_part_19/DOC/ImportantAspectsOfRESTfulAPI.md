...
время все-таки откинуть лень, выделить время и написать «переформатированную» в своей голове статью. 
...
приглашения”. Понятное дело, что никто из Нас не захочет давать свой пароль от ящика на gmail-е Dropbox-у. 

И тут в силу вступает “магия” RESTful веб-сервисов:
1. Используя API, Dropbox делает запрос к веб-сервису Google, и затем, “отправляет” Вас на страницу авторизации Google.
2. Таким образом вы вводите свой пароль в форме у Google, который вас авторизует и дает знать (выдает access_token) Dropbox-у об успешности auth-процесса (authentication).
3. Имея уникальный токен (access_token) и прикрепляя его к каждому запросу в API, Dropbox имеет возможность работать с вашими персональными данными, не зная ничего ровным счетом о вашем пароле (но доступ к данным он имеет ????).
...
Следующим моментом будет создание раутера или ресурса - адреса, по которому можно будет обратиться к данным. 
**Отметим несколько моментов, которые считаются хорошей манерой при составлении адреса**:
...
- *определение формата ответа*: либо в названии URL-е раутера, либо в “системной” переменной;
    - например: `/api/mc/v1/prefs/mydevelopersuid.json` - oDesk использует суффикс ".json" когда требуется ответ в формате JSON, и суффикс ".xml" — в формате XML и тд.;
...
- [REST (by wikipedia.org)](http://en.wikipedia.org/wiki/Representational_State_Transfer)
- [Create, read, update and delete (by wikipedia.org)](http://en.wikipedia.org/wiki/Create,_read,_update_and_delete)
- [JSON (by wikipedia.org)](http://en.wikipedia.org/wiki/JSON)
- [Hypertext Transfer Protocol -- HTTP/1.1 (by w3.org) (This document has been superseded.)](http://www.w3.org/Protocols/rfc2616/rfc2616.html)

---
- См. [комментарии к статье в исходнике](https://habr.com/ru/articles/108993/)
