...
- [GitHub автора](https://github.com/grossws/);
...
Выросло данное API из [JSR 311: JAX-RS: The Java API for RESTful Web Services](https://jcp.org/en/jsr/detail?id=311) и вошло в Java EE 6 (планировалось в Java
EE 5). Как видно из названия, предназначено оно для разработки RESTful веб-сервисов. Основная цель данной статьи — познакомить читателя с основами JAX-RS API. 
Изначально я планировал написать о некоторых проблемах работы форм при использовании JAX-RS сервиса. Не обнаружив на Хабре почти ничего, касающегося данной 
технологии понял, что введением к статье отделаться не удастся.

Будут представлены основы JAX-RS API, реализация от JBoss и дано небольшое введение в клиентскую часть фреймворка Resteasy. Представлено оно набором классов 
в javax.ws.rs.*, в случае современных AppServer'ов предоставляемое окружением (можно использовать `<scope>provided</scope>` в [maven](https://maven.apache.org/). 

Для приложений в сервлет-контейнерах есть несколько реализаций:
- [Apache CXF](https://cxf.apache.org/) (используется в Virgo — наследнице Spring dm Server);
- [Sun/Oracle Jersey](https://eclipse-ee4j.github.io/jersey/) (текущая RI, используется в GlassFish);
- [JBoss Resteasy](https://docs.resteasy.dev/7.0/userguide/) (используется в JBoss AS 7);
- [Restlet](https://www.restlet.org/);

Эта технология позволяет экспортировать методы произвольного бина, используя специальный сервлет, предоставляемый реализаций JAX-RS. Приведу пример, для использования 
в клиентской части выделим интерфейс:
...
Однако, тут есть еще проблема, см. [JAX-RS-Basics-Part-2](../JAX-RS/JAX-RS-Basics-Part-2.md)

---
- См. [комментарии к статье в исходнике](https://habr.com/ru/articles/140181/);
- [GitHub автора](https://github.com/grossws/);
