...
В данном примере @Size и @Digits готовые аннотации из пакета javax.validation, а @PersonAgeConstraint собственная (самописная).
...
Пример реализации класса проверки `PersonAgeConstraintValidator`:
...
т.е. использовать как фильтр. Таким образом проверку можно сделать по этапам. 

Например, разделим проверку Person:
...
В Spring есть свой Validator - [Validator](https://docs.spring.io/spring-framework/docs/current/javadoc-api/org/springframework/validation/Validator.html), как и в Java - [Validator](https://jakarta.ee/specifications/bean-validation/3.0/apidocs/jakarta/validation/validator) и именно его имплементация выполняет проверку данных. Это уже не декларативный подход, в 
нем есть своя гибкость и расширяемость. Для того же bean-a, сделаю туже проверку возраста.
...
Будут выполнены все проверки которые имплементировали [Validator](https://docs.spring.io/spring-framework/docs/current/javadoc-api/org/springframework/validation/Validator.html) для класса Person.
...
**!!! Внимание !!! 
Я почему-то ожидал, что Spring будет выполнять также проверки указанные в аннотациях, но нет, этот
...
Spring validator вызов jakarta.validation.Validator, например так:
...
все проверки для класса Person на [Spring Validator](https://docs.spring.io/spring-framework/docs/current/javadoc-api/org/springframework/validation/Validator.html). Запускаем проверку также через Spring:
...
Теперь в коллекции будут проверки от аннотаций [Java Validator](https://jakarta.ee/specifications/bean-validation/3.0/apidocs/jakarta/validation/validator) и [Spring Validator](org.springframework.validation.Validator) для Person.
