- [См. исходник (ENG)](https://www.baeldung.com/spring-security-csrf)
...
В этом случае злоумышленнику необходимо заставить жертву выполнить аналогичный запрос, но с немного другими параметрами в его теле:
...
В старой конфигурации XML (до Spring Security 4) защита CSRF была отключена по умолчанию, и мы могли включить ее при необходимости через:
...
нашей специальной статье - [CSRF With Stateless REST API](https://www.baeldung.com/csrf-stateless-rest-api), нам нужно 
понять, требуется ли CSRF-защита для нашего API без сохранения состояния.
...
никакого HTML-представления. В этом случае мы можем отправить токен CSRF в файле cookie, используя `CookieCsrfTokenRepository`:
...
Мы обсудили пару атак CSRF и способы их предотвращения с помощью Spring Security, код, [представленный в этой статье, на GitHub](https://github.com/eugenp/tutorials/tree/master/spring-security-modules/spring-security-web-mvc-custom)
