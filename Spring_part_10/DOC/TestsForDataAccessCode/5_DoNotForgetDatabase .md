- [Исходник всего материала (ENG)](https://www.petrikainulainen.net/programming/testing/writing-tests-for-data-access-code-dont-forget-the-database/)

---
- [См. настройка Spring проекта](https://start.spring.io/)

---
### Writing Tests for Data Access Code - Don't Forget the Database - Не забудь о базе данных ***
...
```
    Детерминированность в тестах означает, что при одних и тех же исходных данных тест всегда выдает один
    и тот же результат и проходит (или падает) одинаково, независимо от окружения, времени запуска или
    порядка выполнения.
    
    Для тестов взаимодействия с БД это подразумевает:
    
    - Изоляцию данных: Каждый тест должен начинаться в «чистом» состоянии. Если один тест добавил запись,
                       другой не должен её видеть, чтобы не возникло конфликтов.
    - Управление состоянием: База должна приводиться к эталонному виду перед каждым тестом (через транзакции
                             с откатом (rollback), очистку таблиц или миграции).
    - Независимость от внешних факторов: Тест не должен зависеть от текущего времени сервера (лучше использовать
                                         моки для дат) или случайной сортировки (всегда добавляйте ORDER BY в
                                         запросах, если проверяете списки).
    - Отсутствие побочных эффектов: Тесты не должны изменять глобальные настройки БД, которые могут повлиять
                                    на последующие проверки.
    
    Если тест проходит на вашем компьютере, но падает на CI/CD из-за того, что там уже лежат какие-то данные —
    такой тест считается недетерминированным (или «флапающим», flaky test).
```
...
Я тоже так делал, и это **казалось** мне идеальным решением, потому что:
...
Однако вскоре, я понял, что **у этого подхода есть (как минимум) три проблемы** (зависимость от поведения Hibernate):
- Если база данных создана с помощью Hibernate, мы не можем проверить, что наши сценарии миграции создают рабочую базу данных.
...
- Мы не можем писать необходимые интеграционные тесты для определенных функций, поскольку в нашей базе данных
  отсутствуют требуемые таблицы, триггеры, ограничения или индексы. Это означает, что мы должны протестировать
...
```
    Регрессионное тестирование — это проверка того, что после внесения изменений в код (исправления багов, добавления новых
    функций или рефакторинга) старый, уже работающий функционал не сломался. 
    
    Простыми словами: это способ убедиться, что «одно лечим, а другое не калечим». 
    
    Ключевые особенности:
    - Цель: Обнаружение «регрессии» — ситуации, когда программа деградирует к состоянию с ошибками, которые ранее отсутствовали
            или были исправлены.
    - Когда проводится: При каждом обновлении билда или изменении конфигурации системы.
    
    Методы:
    - Повторное выполнение тестов: Использование уже существующих тест-кейсов для проверки стабильности системы.
    - Автоматизация: Из-за рутинности и частого повторения регрессионные тесты — главные кандидаты на автоматизацию.
    
    Отличие от Re-testing: Повторное тестирование (Re-testing) проверяет только исправление конкретного бага, а регрессионное —
    не «отвалилось» ли что-то в остальной части приложения. 
    
    Когда оно необходимо?
    - После исправления критических дефектов.
    - При добавлении нового функционала, который может повлиять на существующую логику.
    - При оптимизации кода или рефакторинге для улучшения производительности.
    - При изменении окружения (например, обновление версии базы данных или операционной системы). 
```
...
Однако надо быть осторожным, поскольку эти варианты имеют смысл только если:
