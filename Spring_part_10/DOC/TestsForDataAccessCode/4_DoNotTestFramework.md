- [Исходник всего материала (ENG)](https://www.petrikainulainen.net/programming/testing/writing-tests-for-data-access-code-dont-test-the-framework/)

---
- [См. настройка Spring проекта](https://start.spring.io/)

---
### Writing Tests for Data Access Code - Don't Test the Framework - Не надо тестировать фреймворк
...
И все же, ДОЛЖНЫ ЛИ мы написать интеграционные тесты для интерфейса `TodoRepository` и протестировать все эти методы?
...
- **Методы, объявленные интерфейсом CrudRepository, не являются нашим собственным кодом**. Этот код написан и поддерживается
  командой Spring Data, и они гарантируют его работу. Если мы не верим, что их код работает, нам не следует его использовать.
...
**Сквозные тесты (End-to-End или E2E)** — это проверка работы приложения целиком, от начала до конца, путем имитации 
реальных сценариев поведения пользователя. Они проверяют, как взаимодействуют все части системы (фронтенд, бэкенд, БД, 
сторонние сервисы) в условиях, максимально приближенных к реальной эксплуатации. 

**Основные отличия:**

| Характеристика       | Интеграционные тесты                                                                           | Сквозные тесты (E2E) |
|----------------------|------------------------------------------------------------------------------------------------|----------------------|
| Объект проверки      | Взаимодействие между двумя или более конкретными модулями/компонентами.                        | Вся система как единое целое, включая внешние зависимости.|
| Цель                 | Убедиться, что «клей» между частями системы работает корректно и данные передаются без ошибок. | Подтвердить, что пользователь может успешно выполнить бизнес-задачу (например, купить товар). |
| Уровень детализации  | Технический: проверяются интерфейсы API, запросы к БД, интеграция микросервисов.               | Пользовательский: тестируется UI и полный цикл прохождения данных через все слои системы. |
| Сложность и скорость | Выполняются быстрее, их легче изолировать и поддерживать.                                      | Медленные и ресурсоемкие, так как требуют развертывания всего окружения системы. |

**Пример:**

- Интеграционный тест: Проверяет, что сервер корректно записывает данные в базу при получении конкретного API-запроса.
- Сквозной тест: Автоматический скрипт открывает браузер, вводит логин/пароль, добавляет товар в корзину и оформляет заказ, проверяя финальное сообщение об успехе. 
