...
Платформа Spring Security предоставляет классы [WebSecurity](https://docs.spring.io/spring-security/site/docs/current/api/org/springframework/security/config/annotation/web/builders/WebSecurity.html) и [HttpSecurity](https://docs.spring.io/spring-security/site/docs/current/api/org/springframework/security/config/annotation/web/builders/HttpSecurity.html) для предоставления как глобальных, так и специфичных для ресурсов механизмов ограничения доступа к API и активам. Класс WebSecurity помогает настроить безопасность на глобальном уровне, а HttpSecurity предоставляет методы для настройки безопасности для конкретного ресурса.

В этой статье мы подробно рассмотрим использование HttpSecurity и WebSecurity. Также мы увидим различия между этими двумя классами.

---
#### HTTP-безопасность (HttpSecurity)

Класс [HttpSecurity](https://docs.spring.io/spring-security/site/docs/current/api/org/springframework/security/config/annotation/web/builders/HttpSecurity.html) помогает настроить безопасность для определенных HTTP-запросов. Кроме того, он позволяет использовать метод requestMatcher() для ограничения конфигурации безопасности определенной конечной точкой HTTP (endpoint). Так же, он обеспечивает гибкость настройки авторизации для конкретного HTTP-запроса. Мы можем создать аутентификацию на основе ролей с помощью метода hasRole().
...
Здесь мы обеспечиваем дополнительные уровни безопасности запроса, разрешая доступ к конечной точке только пользователям с ролью «ADMIN».
...
| Подход к настройке | Условная конфигурация для каждого ресурса.                   | Глобальная многоразовая конфигурация безопасности. |
